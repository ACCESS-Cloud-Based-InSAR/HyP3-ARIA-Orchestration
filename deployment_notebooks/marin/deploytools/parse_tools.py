...
def continuous_time(product_df, iter_id='fileID', print_msg=False):
    """
    ...
    if len(track_rejected_inds)>0:
        ...
        vprint('All {} scenes are spatially continuous.'.format( \
               len(sorted_products)))
...
def minimum_overlap_query(tiles,
    ...
    vprint("{}/{} scenes rejected for not intersecting with the AOI".format( \
          orig_len-updated_tiles.shape[0], orig_len))

    # group IFGs spatiotemporally
    updated_tiles['ind_col'] = range(0, len(updated_tiles))
    updated_tiles_dict, dropped_indices, gap_scenes_dict = continuous_time(updated_tiles, iter_id, print_msg=print_msg)
    ...
    vprint("")
    vprint("AOI coverage: {}".format(aoi_area))
    vprint("Allowable area of miscoverage: {}".format(250 * azimuth_mismatch))
    vprint("minimum_overlap_threshold: {}".format(minimum_overlap_threshold))
    vprint("")
    ...
    for i in enumerate(updated_tiles_dict):
        ...
        if minimum_overlap_threshold > overlap_area:
            for iter_ind in enumerate(i[1]['ind_col']):
                ...
                vprint("Rejected scene {} has only {}km\u00b2 overlap with AOI".format( \
                    i[1]['fileID'][iter_ind[0]], int(overlap_area)))
                drop_ind = updated_tiles[updated_tiles['ind_col'] == iter_ind[1]].index
                updated_tiles = updated_tiles.drop(index=drop_ind)
    updated_tiles = updated_tiles.reset_index(drop=True)
    vprint("{}/{} scenes rejected for not meeting defined spatial criteria".format( \
          orig_len-updated_tiles.shape[0], orig_len))
    ...
    metadata = {
        'aoi_coverage' : aoi_area, 
        'allow_area_miscoverage' : 250 * azimuth_mismatch,
        'min_ovlp_thresh' : minimum_overlap_threshold,
        'stack_dict' : pd.DataFrame(updated_tiles_dict),
        'gap_scenes_dict' : pd.DataFrame(gap_scenes_dict),
        'rejected_scenes_dict' : pd.DataFrame(rejected_scenes_dict),
    } 


    return updated_tiles, metadata
